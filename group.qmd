...
Creating a palette according to the acronym and the color that matches their logo as far as possible.
...
First of all, to understand what is a low turnout rate, we need to investigate how is the distribution of turnout rate from different municipalities in Spain:

```{r,warning = FALSE}
turnout_rate <- final_election |> 
  mutate(turnout=total_votes/population) |> 
  group_by(municipality) |> 
  reframe(m_turnout=mean(turnout)) |> 
  ungroup()

q25 <- quantile(turnout_rate$m_turnout, probs = 0.25)
q50 <- quantile(turnout_rate$m_turnout, probs = 0.5)
q75 <- quantile(turnout_rate$m_turnout, probs = 0.75)

ggplot(turnout_rate, aes(x = m_turnout)) +
  geom_density(fill = "#ffc403", color = "#c70b1f", alpha = 0.5) +  


  geom_vline(xintercept = q25, color = "#c70b1f", linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = q50, color = "#c70b1f", linetype = "dashed", linewidth = 1) +
  geom_vline(xintercept = q75, color = "#c70b1f", linetype = "dashed", linewidth = 1) +


  geom_text(aes(x = q25, y = 6, label = paste0(percent(q25, accuracy = 0.1))), 
            color = "#c70b1f", angle = 90, vjust = -0.5) +
  geom_text(aes(x = q50, y = 6, label = paste0(percent(q50, accuracy = 0.1))), 
            color = "#c70b1f", angle = 90, vjust = -0.5) +
  geom_text(aes(x = q75, y = 6, label = paste0(percent(q75, accuracy = 0.1))), 
            color = "#c70b1f", angle = 90, vjust = -0.5) +
  ...
  theme_minimal() +
  labs(
    title = "Distribution of Voter Turnout Rate",
    subtitle = "The dashed lines represent the 25th, 50th (median), and 75th percentiles.",
    x = "Turnout Rate (%)",
    y = NULL
  ) +
  theme(
...
As it shows, the 25th percentiles is 71.5% of turnout rate, which should be understand as a relatively low turnout rate in Spain.

According to this criteria, we will investigate the performance of parties in these areas with relatively low turnout:

```{r}
low_turnout <- final_election |> 
  mutate(turnout=total_votes/population) |> 
  filter(turnout < 0.715) |> 
  group_by(party_code) |> 
  mutate(mean_ballot = mean(party_ballots)) |> 
  ungroup() |> 
  mutate(party_code = fct_reorder(party_code, mean_ballot, .desc = TRUE))


ggplot(data=low_turnout, aes(x = reorder(party_code, party_ballots), y = mean_ballot, fill = factor(party_code)))+
  geom_bar(stat = "identity")+
  coord_flip()+
  scale_fill_manual(values = color_mapping)+
  scale_y_continuous(labels = scales::label_number(scale = 1e-6, suffix = "MM")) +
  labs( x= NULL,
        title= "Performance when low turnout rate occurs (<71.5%)",
        subtitle= "We can observe that here the main parties, PP and PSOE, \nare still the biggest winner, and the right-wing-PP's vote surpasses left-wing-PSOE.\nHowever, here the far left party, PODEMOS, \nhas much better performance than the far right party, VOX.")+
  theme_minimal()+
  theme(legend.position = "none",
        plot.title = element_text(size = 12, face = "bold", hjust = 0, margin = margin(b = 5)),
    plot.subtitle = element_text(size = 9, hjust = 0, margin = margin(b = 10)))
```

Finally, we will glimpse at these municipalities with extremely low turnout rates (\< 0.45) to see if the result would be very different:

```{r}
extreme_low_turnout <- final_election |> 
  mutate(turnout=total_votes/population) |> 
  filter(turnout < 0.45) |> 
  group_by(party_code) |> 
  mutate(mean_ballot = mean(party_ballots)) |> 
...
ggplot(data=extreme_low_turnout, aes(x = reorder(party_code, party_ballots), y = mean_ballot, fill = factor(party_code)))+
  geom_bar(stat = "identity")+
  coord_flip()+
  scale_fill_manual(values = color_mapping)+
  labs( x= NULL,
        title= "Performance when extremely low turnout rate occurs (<45%)",
        subtitle= "We can observe that here those Basque nacionalist parties (PNV and EH-BILDU)\nare the biggest winner in low turnout rate regions. \nAnd among those mainsteam parties, \nPSOE has much better performance than PP and PODEMOS \nin in low turnout rate regions.")+
  theme_minimal()+
  theme(legend.position = "none",
        plot.title = element_text(size = 12, face = "bold", hjust = 0, margin = margin(b = 5)),
    plot.subtitle = element_text(size = 9, color = "darkgrey", hjust = 0, margin = margin(b = 10)))

  
```

When we check why the Basque nationalist parties score highest when turnout is low, it is because Pais Vasco (code_community = 14) is the community with the lowest turnout. However, most of the data in this table is from 2008, where there was an election boycott in the community. 624 of the 948 instances of municipalities voting \<45% were in the 2008 election in Pais Vasco.

```{r}
extreme_low_turnout |> 
  group_by(code_community, date_elec) |> 
  summarise(extreme_low_votes = length(code_community)) |> 
