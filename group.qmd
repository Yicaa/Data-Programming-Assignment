...
We created three aggregated data files to work with based on the survey data; election data and election turnout. 

High dimensionality was the key to address, e.g. election data \ with it's 471 columns. We ended up with a 16 column table that removed entirely missing value columns (9). For this, our friends were pivot_longer, distinct and summarise. 
...
There were only 48 municipalities with over 100k people. 2 of the regions, Cádiz and Dos Hermanas had a varying population that was over 100k in some periods and under in others. We created a table with the winners and total % of vote, then plotted over time. PP have a stronghold in some areas. 
...
When PSOE was the winner, PP was consistently the second most voted party in every election. Similarly, when PP was the winner, PSOE was the second most voted in almost all elections, except for 2015 and 2016, where PODEMOS-IU was the second most voted party
```{r}
#| echo: false
...
```
Basque nationalist parties achieve their highest scores in areas with extremely low voter turnout, which is due to the fact that Pais Vasco (code_community = 14) has the lowest turnout among all regions. A significant portion of this data comes from 2008, a year when an election boycott occurred in the region. Of the 948 instances where municipalities recorded a turnout below 45%, 624 took place during the 2008 election in Pais Vasco.
...
# Now breakdown for party  
  ggplot(final_election2, aes(x = log(population), y = vote_pct)) + 
   geom_point(alpha=0.2) + 
   geom_smooth(method = "lm", se = TRUE, aes(color = party_code, group = party_code)) + 
...
# Use regresison to compare the interaction effects on rural areas and party code, and how that impacts vote percentages. 
  rural_compare <- lm(vote_pct ~ log(population) + rural*party_code, data = final_election2) 
...
#Ex post facto tests: further analysis of partisan differences. It finds that the differences are statistically different between groups for rural*party code interaction 
  ...
  summary(anova_model) 
```
Through the analysis of the relationship between census and vote, we first calculated the vote percentage (vote_pct) for each party as the key variable and categorized the population into "rural" (fewer than 10,000 people) and "urban" (10,000 people or more) areas for comparison.

First, I created a chart showing the relationship between the population size (log-transformed) of different constituencies and the vote percentage for a given party. From the chart, it can be observed that although there are some variations in vote percentages across different population sizes, the overall trend is not very strong. Using regression analysis (red line), it was confirmed that changes in population size cannot significantly explain variations in a party's vote percentage. The regression analysis showed no significant overall relationship between log-transformed population size and vote percentage, as the p-value was much greater than 0.05. Then, I used visualization to clearly demonstrate how different parties performed across constituencies of varying population sizes. For each party, I analyzed the relationship between population size (log-transformed) and vote percentage. It was found that PP showed a negative correlation between vote percentage and population size (downward slope), indicating that PP performed better in small constituencies (areas with smaller populations). On the other hand, PSOE, PODEMOS-IU, and OTHER showed a positive correlation (upward slope), indicating these parties performed better in larger constituencies (areas with larger populations).

I then used regression analysis to study the interaction effects of rural areas (rural) and party (party_code) on vote percentage. The analysis revealed that rural areas had a significant negative impact on the overall vote percentage, while different parties performed significantly differently in rural and urban areas. For example, the CS party performed better in urban areas but weaker in rural areas, whereas parties like PSOE and PODEMOS-IU performed better in urban areas. The direct impact of population size on vote percentage was not significant.

Next, I used the code to generate the vote distribution of different parties in rural and urban areas and examined its relationship with population size. First, the code categorized constituencies as "rural" (population less than 10,000) or "urban" and then calculated the total votes for each party in both rural and urban areas. Visualization of the vote proportions of different parties in rural and urban areas revealed that PP had a higher vote proportion in rural areas, while PSOE and PODEMOS-IU had higher vote proportions in urban areas. This indicates significant differences in party support between rural and urban areas.

Finally, I conducted a two-way ANOVA to examine the impact of rural classification (rural) and party (party_code) on vote percentage (vote_pct), as well as the interaction between the two. The analysis showed that the standalone effect of rural classification on vote percentage was not significant. However, the interaction between party and rural classification revealed significant differences in party support between rural and urban areas.
...
### Q4 : Relationship between census and vote (urban vs. rural)
Different parties show varying trends in vote percentages across population scales (log-transformed), such as PP performing better in smaller constituencies (lower population), while PSOE and PODEMOS-IU perform better in larger constituencies (higher population).
```{r}
#| echo: false
# Now breakdown for party  
  ggplot(final_election2, aes(x = log(population), y = vote_pct)) + 
...
### Q4 : Relationship between census and vote (urban vs. rural)
Different parties show significant differences in support between rural and urban areas. For example, PP has higher support in rural areas, while PSOE and PODEMOS-IU have stronger support in urban areas.
```{r}
#| echo: false
# Count the proportion of votes received in each area, then compare for statistically significant differences. 
  party_votes <- final_election2 |>  
...
There does not seem to be any party that is consistently overestimated or underestimated by a big amount. Surveys have only difficulties in capturing support to smaller parties, category "OTHER", but this is plausible as often survey give a limited number of responses available, and in general support for smaller parties is more difficult to detect

I wanted also to investigate whether surveys conducted short before an election are more reflective of the results of that election. I would expect that they indeed should produce a lower polling error, not because surveys conducted long before the election are necessarily wrong, but because opinion of people can change.

This relationship indeed exists (is significant), but explains very little of the variability of the MAE of polls. This also seems to suggest us that people's voting intentions are quite stable in Spain.

To try to explain the variability of polling errors I therefore explored whether more accurate polls were the ones conducted on a bigger sample size. Again, this relationship exists (polls with a bigger sample size seem to be more precise), it is significant, but it explains very little of the total variability of the polling error.

So in the next question we will explore whether it's the case that simply there are some pollster that are able to predict people's voting intention better than others.

-------------------

### Q5 : Polling error
Are some parties consistently underestimated or overestimated by surveys?
```{r}
#| echo: false
...
```{r}
#| echo: false
surveys_date <- final_surveys |> 
  left_join(election_q5, by = join_by(date_elec, party_code)) |> 
  select(date_elec, id_pollster, media, field_date_from, field_date_to, size, party_code, votes, share_votes) |> 
  rename(voting_intention = votes, actual_votes = share_votes)

survey_mae <- surveys_date |> 
  mutate(abs_error = abs(voting_intention - actual_votes)) |>
  group_by(id_pollster, date_elec, media, field_date_from, field_date_to, size) |>
  summarise(mae = mean(abs_error), .groups = "drop") |> 
  mutate(days_to_election = as.numeric(difftime(date_elec, field_date_to, units = "days")))

ggplot(survey_mae, aes(x = days_to_election, y = mae)) +
  geom_point(color = "black", alpha = 1) + 
  geom_smooth(method = "lm", se = TRUE, color = "red") + 
  scale_y_continuous(limits = c(0,5), expand = expansion(add = c(0,0))) +
  labs(
    title = "Does proximity to the election explain polling errors?",
    subtitle = "Each point represent a national survey conducted for any election between 2008 and 2019",
    x = "Number of days before an election \u2192",
    y = "Surveys's Mean Absolute Error"
  ) +
  scale_x_reverse(limits = c(1500,0), expand = expansion(add = c(0,0))) +
  theme_minimal() +
...
#| echo: false
#| eval: false
summary(lm(mae ~ days_to_election, data = survey_mae))
```
...
```{r}
#| echo: false
surveys_100 <- final_surveys |> 
  filter(abs(as.numeric(difftime(date_elec, field_date_from, units = "days"))) <= 60) |> 
  left_join(election_q5, by = join_by(date_elec, party_code)) |> 
  select(date_elec, id_pollster, media, field_date_from, field_date_to, size, party_code, votes, share_votes) |> 
  rename(voting_intention = votes, actual_votes = share_votes)

survey_mae <- surveys_100 |> 
  mutate(abs_error = abs(voting_intention - actual_votes)) |>
  group_by(id_pollster, date_elec, media, field_date_from, field_date_to, size) |>
  summarise(mae = mean(abs_error), .groups = "drop")

ggplot(survey_mae, aes(x = size, y = mae)) +
  geom_point(color = "black", alpha = 1) + 
  geom_smooth(method = "lm", se = TRUE, color = "red") + 
  scale_y_continuous(limits = c(0,4), expand = expansion(add = c(0,0))) +
  labs(
    title = "Does sample size explain polling error?",
     subtitle = "Each point represent a national survey conducted for any election between 2008 and 2019",
    x = "Survey's Sample Size",
    y = "Surveys's Mean Absolute Error"
  ) +
  theme_minimal() +
  theme(panel.grid.minor = element_blank(),
...
#| echo: false
#| eval: false
summary(lm(mae ~ size, data = survey_mae))
```
...
ggplot(wmae, aes(x = pollster, y = WMAE, fill = WMAE)) +
  ...
  scale_y_continuous(labels = scales::comma) +
  scale_fill_gradient2(low = "green", mid = "#ffc403", high = "#c70b1f", midpoint = 1.8) 
...
```
The analysis of prediction bias trends over time highlights important patterns in the consistency of survey projections across election periods, as shown below:
1.PP:
2016: Surveys underestimated support (bias ~-4%), likely due to voter uncertainty.
2018: Bias shifted to nearly +4%.
2020: Bias approached zero, showing improved accuracy.
2.PSOE:
2012: Surveys overestimated support (bias ~+4%).
2017-2020: Bias dropped to near zero, reflecting more reliable predictions.
3.VOX:
2017-2018: Accurate predictions with bias near zero.
2020: Bias became volatile due to VOX’s rapid growth and survey challenges.
4.CS:
2017-2018: Stable bias indicated accurate predictions.
2020: Bias increased, reflecting shifts in voter support.
Other and Smaller Parties:
5.Regional parties like BNG and EH-BILDU had stable, mild biases.
6.Newer parties like PODEMOS-IU faced significant fluctuations, especially in 2016.
Overall: Major parties improved prediction accuracy, while newer and emerging parties faced greater challenges. Smaller parties were more consistent.

---

### Q7 : Survey projection accuracy over time
Trends in survey prediction biases over time, focusing on bias evolution and accuracy for traditional, emerging, and regional parties. Insights for improving survey methods.
```{r}
trend_data |> 
  ggplot(aes(x = date_elec, y = prediction_bias, color = party_code, group = party_code)) +
  geom_line(size = 1) +
