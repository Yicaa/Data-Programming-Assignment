...
#data visualization
ggplot(low_turnout_summary, aes(x = reorder(party_code, -vote_share), y = vote_share, fill = party_code)) +
  geom_bar(stat = "identity", color = "black", width = 0.7) +
  labs(
    title = "Party Vote Share under Low Turnout (<25%)",
    x = "Party",
    y = "Vote Share (%)"
  ) +
  theme_minimal() +
...
```
Based on the chart, **Partido Popular(PP)** and **Partido Socialista Obrero Español(PSOE)** are the parties that benefit the most under low turnout conditions. These two parties dominate the vote share, each capturing a significant proportion of the total votes in scenarios where voter turnout is below the 25th percentile.

This suggests that these parties likely have:
1. A Loyal Voter Base: Their supporters are more consistent and motivated to vote even under low turnout conditions.
2. Established Organizational Structures: They might have stronger mechanisms to mobilize voters or a more stable electorate compared to smaller or less prominent parties.
...
tidy_election2 <- tidy_election |> 
  mutate(rural = ifelse(population < 10000, 1, 0))  # 1 is rural, 0 is urban

# Counting the total number of votes for urban and rural political parties
party_votes <- tidy_election2 |> 
  group_by(rural, party_main) |> 
  summarise(total_votes = sum(party_ballots, na.rm = TRUE), .groups = "drop") |> 
  arrange(rural, desc(total_votes))
party_votes

# Visual analysis of urban and rural party performance
ggplot(party_votes, aes(x = as.factor(rural), y = total_votes, fill = party_main)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Party Performance in Rural vs Urban Areas",
       x = "Area Type (1 = Rural, 0 = Urban)",
       y = "Total Votes",
       fill = "Party Name") +
  theme_minimal()

# Regression analysis of the effect of urban/rural and population size on vote choice
model <- lm(party_ballots ~ population + rural, data = tidy_election2)
summary(model)
```
ggplot2 graph presents the total votes received by each political party in rural (1) and urban (0) areas, clearly highlighting the dominance of major parties such as PARTIDO POPULAR and PARTIDO SOCIALISTA OBRERO ESPAÑOL in both settings. It reveals that urban areas generally have higher total votes across all parties, but the two dominant parties maintain strong performance in rural regions as well. Smaller parties, such as MÁS PAÍS, show a more noticeable presence in urban areas, indicating that urban voters may exhibit more diverse political preferences compared to rural voters who seem to concentrate their support on larger, established parties.

Based on a linear regression model, evaluates how population size and rural classification influence vote counts. The results indicate that population size has a positive and significant relationship with vote totals, implying that larger populations tend to generate more votes. Additionally, the rural variable is also statistically significant, with rural areas contributing an average of 121 more votes than urban areas, suggesting that rural constituencies may slightly favor higher voter turnout or more concentrated political support. However, with an R-squared value of 0.3098, the model explains about 31% of the variation in vote totals, leaving room for other factors such as economic, social, or cultural differences to further explain voting behavior.

In analyzing the relationship between census and vote, the results suggest that population size directly impacts vote totals, as larger areas generate more votes. The rural variable also indicates a modest but statistically significant effect, showing that rural areas may contribute slightly higher vote counts compared to urban areas. The data supports the hypothesis that certain parties, particularly larger and more established ones, perform well in both rural and urban areas, although urban regions exhibit greater political diversity due to the presence of smaller parties.
...
#Relationship between census and vote(Regression analysis)
model <- lm(party_ballots ~ population, data = final_election2)#Examining the relationship between population size and votes
summary(model)
...
ggplot(party_votes, aes(x = as.factor(rural), y = total_votes, fill = party_code)) +
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Party Performance in Rural vs Urban Areas",
       x = "Area Type (1 = Rural, 0 = Urban)",
       y = "Total Votes",
...
final_election2$party_main <- as.factor(final_election2$party_main)
anova_model <- aov(party_ballots ~ rural * party_main, data = final_election2)
summary(anova_model)

#Ex post facto tests: further analysis of partisan differences
TukeyHSD(anova_model, "party_main")
```
The results of the regression analysis show that there is a significant positive correlation (p < 0.001) between population size and party_ballots, i.e., the larger the population size, the greater the number of ballots. The model explains about 29.49% of the variation in ballots (R² = 0.2949), indicating that population size is an important influence on the distribution of ballots, but there are other factors that are not included in the model.

The ggplot graph shows the distribution of total votes for each political party in rural (1) and urban (0) areas. The data show that PARTIDO POPULAR and PARTIDO SOCIALISTA OBRERO ESPAÑOL are dominant in both rural and urban areas, but that urban voters show greater political diversity, while rural voters are more concentrated in their support for the main political parties, with a more pronounced tendency to support specific parties.

The results of the Two-way ANOVA indicate that both urban-rural categorization and party type significantly affect the distribution of votes, while there is an interaction effect suggesting that some specific parties perform more prominently in rural or urban areas.
TukeyHSD post hoc test results show that there is a significant difference (p < 0.05) in the number of votes across parties, with PARTIDO POPULAR and PARTIDO SOCIALISTA OBRERO ESPAÑOL in particular being significantly ahead of the other parties in most comparisons, suggesting that they have a stronger vote advantage in both urban and rural areas. In addition, interaction effects further support the idea that certain parties perform more prominently in specific regions.
...
### 7. Jacklyn and Linghan
Do the survey projections of vote shares correspond to those of the actual elections? If there was a deviation, in which parties was it more pronounced?

```{r}
# Define a function to process election and survey data
process_election_data <- function(final_election, final_surveys) {
  # Convert date columns to Date format
  final_election$date_elec <- as.Date(final_election$date_elec)
  final_surveys$date_elec <- as.Date(final_surveys$date_elec)

  # Aggregate election data by party and calculate actual vote percentage
  election_aggregated <- final_election %>%
    group_by(party_code, party_main) %>%
    summarise(
  ...
  # Aggregate survey data by party and calculate average predicted vote percentage
  survey_aggregated <- final_surveys %>%
    group_by(party_code, party) %>%
    summarise(predicted_vote_share = mean(votes, na.rm = TRUE),
               .groups = "drop")

  # Merge aggregated election and survey data on party codes
  merged_data <- merge(election_aggregated, survey_aggregated, by = "party_code")
...
merged_data <- process_election_data(final_election, final_surveys)

# Visualize the results
merged_data %>%
  mutate(party_main = fct_reorder(party_main, prediction_bias)) %>% # Reorder parties by bias
  ggplot(aes(x = party_code, y = prediction_bias, fill = prediction_bias > 0)) +
  geom_bar(stat = "identity", show.legend = FALSE) +
  coord_flip() +
  labs(
    title = "Prediction Bias by Party",
    x = "Party",
    y = "Prediction Bias (%)",
    fill = "Positive Bias"
  ) +
...
The survey projections of vote shares did not perfectly correspond to the actual election results, as evidenced by the Prediction Bias by Party chart. Prediction bias is the deviation between the survey-predicted vote shares and the actual vote shares observed in the election.

From the visualization:
Positive Bias (overestimation):
The CS party experienced a significant positive bias, meaning that its vote share was overestimated in the surveys compared to the actual election results.

Negative Bias (underestimation):
Several parties, such as PSOE, PP, and PODEMOS-IU, show a negative bias, indicating that their actual vote shares were underestimated in the surveys.
The negative bias for PODEMOS-IU appears particularly pronounced, suggesting a substantial underestimation of their support.

Smaller Bias:
Certain smaller parties (e.g., ERC, EH-BILDU, and MP) display relatively minor deviations, suggesting better alignment between survey predictions and actual results.
