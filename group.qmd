...
survey_q5 <- final_surveys |> 
  # Should I drop NA turnout
  #drop_na(turnout) |>
  summarise(avg_vote = mean(votes), .by = c(date_elec, party_code))

survey_q5 |> 
  summarise(sum(avg_vote), .by = date_elec) # Why those this happen? is it fine?

survey_turnout <- final_surveys |> 
  drop_na(turnout) |> 
  summarise(avg_turnout = mean(turnout), .by = date_elec)

survey_q5 <- survey_q5 |> 
  left_join(survey_turnout, by = join_by(date_elec))

election_q5 <- final_election |>
  summarise(party_votes = sum(party_ballots), .by = c(date_elec, party_code))

# Maybe I should not use total votes but valid votes (blank + party votes)
turnout_q5 <- turnout |> 
  summarise(tot_votes = sum(valid_votes), tot_pop = sum(population),
            .by = c(date_elec)) |> 
  mutate(turnout = (tot_votes/tot_pop)*100)

election_q5 <- election_q5 |> 
  left_join(turnout_q5, by = join_by(date_elec)) |> 
  mutate(share_votes = (party_votes/tot_votes)*100)

final_q5 <- survey_q5 |> 
  left_join(election_q5, by = join_by(date_elec, party_code)) |> 
  select(-tot_votes, -tot_pop, -party_votes) |> 
  mutate(polling_error = avg_vote - share_votes)

polling_err <- final_q5 |> 
  summarise(avg_err = mean(polling_error), .by = party_code)

#Align with palette used for other questions
custom_colors <- c("PSOE" = "#f20400", "PP" = "blue", "PODEMOS-IU" = "#663278",
                   "CS" = "#e9550d", "VOX" = "#76b829", "MP" = "#00e1c5", 
                   "CIU" = "darkblue", "BNG" = "lightgreen", "EH-BILDU" = "#0157a1",
                   "OTHER" = "black", "ERC" = "yellow", "EAJ-PNV" = "pink")
...
# Should we rescale the analysis??


# Plot polling error vs turnout/turnout difference? --> useless
# turnout_analysis <- final_q5 |> 
#   reframe(turnout_diff = unique(avg_turnout)- unique(turnout), 
#           polling_diff = mean(polling_error),
#           .by = date_elec)
# ggplot(turnout_analysis, aes(x=turnout_diff, y=polling_diff))+
#   geom_point()
# 


# Plot avg polling error dynamically by party and election
```
