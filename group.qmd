...
Low-Turnout Areas (<70.7%): PSOE and PP lead, with smaller parties like VOX, CS, and EH-BILDU moderately present. Regional and new parties gain little traction.
```{r}
#| echo: false
ggplot(low_turnout_summary, aes(x = party_code, y = vote_share, fill = party_code)) +
  geom_bar(stat = "identity", color = "black", width = 0.7) +
...
ggplot(final_election2, aes(x = log(population), y = vote_pct)) + 
  ...
  geom_smooth(
    method = "lm", 
    se = TRUE, 
    aes(color = party_code, group = party_code) 
  ) + 
  facet_wrap(~party_code) + 
  scale_color_manual(values = party_colors) + 
  labs(
    title = "Votes vs. Population by Party", 
    x = "Population log", 
    y = "Pct of votes for party",
    color = "Party" 
  ) + 
...
ggplot(party_votes, aes(x = party_code, y = total_votes, fill = as.factor(rural))) + 
   ...
   labs(
     title = "Proportion of votes for parties in Rural vs Urban Areas", 
     x = "Party", 
     y = "Total Votes", 
     fill = "Rural status"
   ) + 
...
party_votes2 <- party_votes |> 
  group_by(party_code) |> 
  summarize(
    rural_votes = sum(total_votes[rural == 1]),
    urban_votes = sum(total_votes[rural == 0]),
    total_votes_party = rural_votes + urban_votes,
    rural_proportion = rural_votes / total_votes_party,
    .groups = "drop") |> 
  select(party_code, rural_proportion)

party_votes2 <- party_votes |> 
  left_join(party_votes2, by = join_by(party_code))

ggplot(party_votes2, aes(x = reorder(party_code, rural_proportion, decreasing = TRUE), y = total_votes, fill = as.factor(rural))) + 
   geom_bar(stat = "identity", position = "fill") + 
   labs(
     title = "Proportion of votes for parties in Rural vs Urban Areas", 
     x = "Party", 
