...
=======
author: "Marco Boso (100535153), Diego Paroli 100554973, Yijia Lin 100452242, Isabel Monge 100542532..."
>>>>>>> Stashed changes
format:
  html:
    theme: [style.scss]
...
Modify main branch to see how to resolve conflicts
...
```{r}
# Filter data for elections between 2008 and 2019
filtered_data <- election_data |>
  filter(anno >= 2008 & anno <= 2019)

# Reshape the dataset into long format
tidy_election <- filtered_data |>
  pivot_longer(
    cols = starts_with("votos_"),  # Include all vote-related columns
    names_to = "party_raw",        # Create a column for raw party names
    values_to = "votes"            # Create a column for vote counts
  )

# Group parties using predefined patterns
...
tidy_election <- tidy_election |>
  select(tipo_eleccion, anno, mes, vuelta, party, votes)
