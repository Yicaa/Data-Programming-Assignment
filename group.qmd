...
The election_data file is large and requires quite extensive cleaning to make it "tidy". We will tidy the data to try make it most useful for future analysis. The election data starts off with 48,737 rows and 471 rows. Reducing the number of columns is a clear priority. 
...
We now have a table with 21,785,439 rows and 17 columns. This is more clean than previously, but we still need to aggregate of our party variables into the main party groups. We will do this by matching on the 
...
Now we will include some addiitonal information that will make the analysis potentially easier later, including province and valid vote counts from our data:

``` {r join-provnames}
# create municipal code to join on municipal names. 
election_pivot <- election_pivot |>
  mutate(cod_mun = paste(codigo_ccaa, codigo_provincia, codigo_municipio, sep="-"), # create municipo code to join
...
# check quality of the join and whether NA's have been introduced as municpality names
...
For Isabelle, Marco and Brad post-cleaning, describe out data status after cleaning at this stage:
