...
ggplot(low_turnout_summary, aes(x = party_code, y = vote_share, fill = party_code)) +
  ...
  geom_text(aes(label = round(vote_share, 1)), vjust = -0.5, size = 3) +  
  geom_hline(yintercept = mean(low_turnout_summary$vote_share), linetype = "dashed", color = "red") + 
  labs(
    ...
    caption = "Dashed line represents the average vote share.",
    x = "Party",
    y = "Vote Share (%)"
  ) +
  theme_minimal() +
...
average_vote_share <- mean(extreme_low_turnout_summary$vote_share)

ggplot(extreme_low_turnout_summary, aes(x = party_code, y = vote_share, fill = party_code)) +
  geom_bar(stat = "identity", color = "black", width = 0.7) +
  geom_hline(yintercept = average_vote_share, linetype = "dashed", color = "red") + 
  geom_text(aes(label = round(vote_share, 1)), vjust = -0.5, size = 3) + 
  labs(
    title = "Vote Share by Party in Extreme Low Turnout Areas (<45%)",
    subtitle = "Dashed line represents the average vote share",
    x = "Party",
    y = "Vote Share (%)"
  ) +
  theme_minimal() +
...
# Use regression to compare the interaction effects on rural areas and party code, and how that impacts vote percentages. 
