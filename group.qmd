...
#Loop through elections and create a plot for each election
...
for (date in  unique_dates) {
  ...
  plot <- ggplot(data_filtered, aes(x = population_category, 
    ...
    labs(
      title = paste("Second Party by Population for Election on", format(current_date, "%Y-%m-%d")),
...
#Loop through elections and create a plot for each election for PP
...
From the analysis, we found that PP was the second party whenever PSOE was the winner in all the elections. Similarly, PSOE was the second party in almost every election where PP was the winner, except for the elections of 2015 and 2016, where PODEMOS-IU was the second party.
...
### Q2 : Primary Runner-Up Parties to PSOE and PP in Elections
The original code uses a loop to analyze the second-party votes across all elections, breaking down the results by the size of the cities (in terms of population) where people voted.
